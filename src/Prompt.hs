...
import Graphics.Gloss (Picture (..), blank, pictures, black, blue,red, rectangleSolid, white, translate)
...
renderPrompt s p = do
    ...
    return $ pictures ([box,t,if showConfirmButton p then drawnOkayButton else blank,if showCloseButton p then drawnCloseButton else blank] 
