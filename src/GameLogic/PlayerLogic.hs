...
import GameLogic.GameLogic (calculateGameSpeed, fruitPlayerCollision, getFruitScore)
import GameLogic.GhostLogic (hasFrightenedGhost, setGhostBehaviour)
...
import State
  ( GameState(fruitEaten, gMap, killingSpree, level, pelletCount,
          pellets, player, score, totalPelletCount)
  , GhostBehaviour(..)
...
getPlayerVelocity s
  | hfg && isOnPellet = frightPacDotSpd
  | hfg = frightPacSpd
...
updatePlayerPosition dt s
  ...
  where
    ...
    canTurn = maybe False (\d -> isCellCond m (not . cellHasTypes [Wall, GhostWall]) (currentGridPos + dirToVec2 d)) bufferedInput
    newDir
    ...
    (newScore, newPelletCount, newMap)
      ...
      | ctype == PowerUp = (oldScore + 50, oldPelletCount + 1, clearCell m cLoc)
      | otherwise = (oldScore, oldPelletCount, m)
    respawnPellets = newPelletCount >= totalPelletCount gs
    eatFruit = fruitPlayerCollision s dims
    pelletGameState
      | newPelletCount >= totalPelletCount gs = gs {pelletCount = 0, gMap = setCells newMap (pellets gs), fruitEaten = False, level = level gs + 1}
        ...
        s
          { gameState =
              pelletGameState
                { score =
                    if eatFruit
                      then newScore + getFruitScore s
                      else newScore
                , killingSpree =
                    if ctype == PowerUp
                      then 1
                      else killingSpree gs
                , player =
                    ps
