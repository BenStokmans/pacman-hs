...
import GameLogic.GameLogic (calculateGameSpeed, ghostPlayerCollision)
...
import GameLogic.Pathfinding (getAdjacentVecs, getDirectionsLimited, getTraveledDirection, vec2Dist)
import GameLogic.Struct ( GhostType(..), ghosts )
import Rendering ( screenToGridPos )
import State
  ( GameState(blinky, clyde, gMap, godMode, inky, killingSpree, level,
          lives, pauseGameTimer, pelletCount, pinky, player, score)
  , GhostActor(..)
  , GhostBehaviour(..)
  , GlobalState(clock, gameLevel, gameState, settings)
  , Player(..)
  , Settings(ghostBlinkLength, ghostRespawnTimer, ghostStuckTimeout)
  , gameGridInfo
  , getGhostActor
  , ghostActors
  )
...
updateGhostTarget ghost s
  ...
  | ghostState == Frightened && mustPathfind = do
    let allowedDirections =
          filter
            (\d -> isCellCond m (not . cellHasType Wall) (currentGridPos + dirToVec2 d))
            (delete (oppositeDirection currentDirection) allDirections)
    rDir <-
      if null allowedDirections
        then do
          return currentDirection
        else getRandomElement allowedDirections
...
updateGhostGameState gs ghost
  | ghostT == Blinky = gs {blinky = ghost}
...
getBehaviour clock level
  | level == 1 && clock < 7 = Scatter
...
updateGhost gs dt ghost l
  | gUpdate ghost > ghostStuckTimeout (settings gs) = updatedGhost {gUpdate = 0, lastDirChange = outOfBounds}
  ...
  where
    ...
    progressedAnimClock
      | animClock > 0 = animClock - dt
      | otherwise = 0
    (blink, newAnimClock)
      | not stayFrightened = (gBlink ghost, progressedAnimClock)
...
setGhostBehaviour s ghost b =
  updateGhostGlobalState s ghost {gCurrentBehaviour = b, gDirection = direction, lastDirChange = newDirChange, gBlink = False, gAnimClock = 0}
  where
    ghostT = ghostType ghost
    direction
      | b == Frightened = oppositeDirection $ gDirection ghost
      | otherwise = gDirection ghost
    newDirChange
      | b == Frightened = screenToGridPos (gameGridInfo s) (gLocation ghost)
...
getGhostVelocity s ghost
  | behaviour == Respawning = 0
...
updateGhostVelocity s ghost =
  let nv = getGhostVelocity s ghost
   in updateGhostGlobalState
        s
        ghost
          { gVelocity = nv
          , lastDirChange =
              if gVelocity ghost == 0 && nv /= gVelocity ghost
                then outOfBounds
                else lastDirChange ghost -- make sure ghosts don't get stuck in spawn
          }
...
updateGhostPosition dt s ghost = s {gameState = newGameState}
  where
    ...
    (newDir, newChange)
      ...
      | gTarget ghost == currentGridPos && null allowedDirections && isOutOfBounds m (currentGridPos + dirToVec2 currentDirection) =
        (currentDirection, oldChange)
    ...
    newGhost =
      ghost
        { gLocation = finalLocation
        , gDirection = newDir
        , lastDirChange = newChange
        , gUpdate =
...
checkCollisionsForGhost s ghost
  | godMode gs = s
  | colliding && gCurrentBehaviour ghost == Frightened =
    deadGhostGS {gameState = (gameState deadGhostGS) {score = score gs + 200 * (2 ^ (ks - 1)), killingSpree = ks + 1, pauseGameTimer = 1}}
  | colliding && gCurrentBehaviour ghost == Respawning = s
  | colliding =
    foldr (\g ts -> updateGhostGlobalState ts (getGhostActor ts g) {gLocation = gridToScreenPos gi $ getGhostSpawnPoint level g}) deadPlayerGS ghosts
  | otherwise = s
  where
    gi = gameGridInfo s
    ...
    respawnGhost =
      ghost
        { gLocation = gridToScreenPos gi $ getGhostSpawnPoint level ghostT
        , gCurrentBehaviour = Respawning
        , gFrightenedClock = 0
        , gDirection = head allowedDirections
        , lastDirChange = spawnPoint
        }
    deadGhostGS = updateGhostGlobalState s respawnGhost
    deadPlayerGS
      | lives gs == 1 = s {gameState = gs {lives = 0, pauseGameTimer = 2, player = (player gs) {pLocation = (-1000, -1000)}}}
      | otherwise =
        s
          { gameState =
              gs
                ...
                , player =
                    (player gs)
                      { pLocation = gridToScreenPos gi $ getSpawnPoint level
                      , pDirection =
                          fromMaybe North $
                          headMaybe $
                          map (getTraveledDirection (getSpawnPoint level)) $
                          filterLevelVec2s level (not . cellHasTypes [Wall, GhostWall]) $ adjacentVecs (getSpawnPoint level)
                      }
                }
          }
