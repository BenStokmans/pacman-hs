...
drawParticles s =
  Color
    (makeColor 0 0 1 0.4)
    (pictures
       (map (\((x, y), r) -> translate x y (circleSolid r)) (particles s)))
...
updateParticles f s
  | c - p < 0.4 = do
    return
      s {particles = map (\((xp, yp), r) -> ((xp, yp + (f * animConst)), r)) ps}
  | otherwise = do
    xp <- getRandomR (-x2 + x2 / 12, x2 - x2 / 12)
    r <- getRandomR (2 :: Float, 8)
    return
      s
        { gameState = gs {prevClock = c}
        , particles = ((xp, -y2 - (r / 2)), r) : ps
        }
  where
    gs = gameState s
    c = clock s
...
renderStartMenu s = do
  ...
  drawnStartButton <-
    defaultButton
      ...
      (mousePos s)
  drawnQuitButton <-
    defaultButton quitButton (l (emuFont (assets s))) "Quit game" (mousePos s)
  let mEmu = m (emuFont (assets s))
  subTitle <-
    renderString (0, 160) mEmu red "By Ben Stokmans and Geerten Helmers"
  drawnNewMapButton <-
    defaultButton newMapButton mEmu "Create new map" (mousePos s)
  drawnEditMapButton <-
    defaultButton editMapButton mEmu "Edit existing map" (mousePos s)
  return
    (pictures
       [ drawParticles s
       , titleBg
...
emptyMap w h =
  ...
  concatMap
    (\x ->
       [ Cell Wall (Vec2 (fromInteger x :: Float) 0)
       , Cell Wall (Vec2 (fromInteger x :: Float) (h - 1))
       ])
    [0 .. (round w - 1)] ++
  concatMap
    (\y ->
          ...
          , prompt =
              Just
                defaultPrompt
                  ...
                  , closeAction =
                      \state _ -> state {route = StartMenu, prompt = Nothing}
                  }
          }
  ...
  | rectangleHovered (mousePos s) editMapButton = do
    ...
    file <-
      openFileDialog
        ...
        False
    map@(LevelMap w h _) <-
      maybe (do return $ emptyMap 25 25) (readLevel . head) $
      (Just . map unpack) =<< file
    return $
      if isJust file
        then s
               { editorLevel = map
               , route = EditorView
               , settings = (settings s) {editorGridDimensions = Vec2 w h}
               }
        else s
