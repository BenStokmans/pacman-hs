...
import           Graphics.Gloss.Interface.IO.Game (Event (..), Key (..),
                                                   ...
                                                   MouseButton (..),
                                                   SpecialKey (..))
import           Map                              (WallType, getGhostSpawnPoint,
                                                   getSpawnPoint, processWalls,
                                                   wallToSizedSection)
...
import           Text.Printf                      ()
import           Views.GameView                   (cellSize, debugGrid,
                                                   drawGhost, drawGrid, drawMap,
                                                   drawPlayer, gridSizePx,
                                                   gridToScreenPos,
                                                   screenToGridPos)
...
renderEditorView gs = do
    ...
    instructionText <- renderStringTopLeft (-390,60) sEmu white "Select tool\nUsing arrow keys\n \nThen select\nAny square \nin the maze\nusing the mouse\n \nUse the space key\nto switch ghosts\n  \nLeft click: place\nRight click: del.\n    \nThe keys:\nW,A,F and S \nalso activate \ntheir respective\ntools."
    ...
    let tools = pictures [toolsText,toolArrow,wallButton,wallToolText,spawnButton,spawnToolText,foodToolText,foodButton,appleToolText,appleButton,ghostToolText,ghostButton,instructionText]
    ...
    let gridEditor = pictures [pictures cs,if not rightDown then hoveredCell else blank,editorGrid gs]

    let wallsPreview = drawMap gs { cachedWalls = processWalls level } level (c,r) (w,h)
    let pacmanPreview = if v == outOfBounds then blank else drawPlayer gs dims $ gridToScreenPos dims (getSpawnPoint level)
    let ghostPreviews = pictures $ map ((\(t,v) -> if v == outOfBounds then blank else drawGhost gs t dims (gridToScreenPos dims v)) . (\t -> (t,getGhostSpawnPoint level t))) ghosts
    let gridPreview = pictures [wallsPreview,pacmanPreview,ghostPreviews]

    let grid = translate (mx/2) 0 $ if previewEditor gs then gridPreview else gridEditor
...
getNextCircular v xs | null rest = head xs
                     | otherwise = head rest
                where
...
handleInputEditorView (EventKey (SpecialKey KeySpace) _ _ _) gs@(GlobalState {editorGhost = et, editorTool = GhostTool}) = do return gs {editorGhost = nextGhost et}
